...
export const useSpacexStore = defineStore("spacex", () => {
  ...
  const loadingStates = ref<Record<string, LoadingState>>({
    ...
    starlink: {
      isLoading: false,
      loadingMessage: "Cargando satélites Starlink...",
    },
    upcomingLaunches: {
      isLoading: false,
      loadingMessage: "Cargando próximos lanzamientos...",
    },
  });
  ...
  const setLoading = (key: string, isLoading: boolean, message?: string) => {
    loadingStates.value[key] = {
      isLoading,
      loadingMessage:
        message || loadingStates.value[key]?.loadingMessage || "Cargando...",
    };
  };
  ...
  const fetchUpcomingLaunches = async () => {
    ...
    try {
      ...
      const apiError = createApiError(err, "próximos lanzamientos");
