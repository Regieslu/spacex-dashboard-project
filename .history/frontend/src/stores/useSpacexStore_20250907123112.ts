...
import { getLaunches, getRockets, getStarlink, getUpcomingLaunches } from "../api";
...
export const useSpacexStore = defineStore("spacex", () => {
  ...
  const isUpcomingLaunchesOutdated = computed(() => {
    ...
    return Date.now() - lastFetchUpcomingLaunches.value.getTime() > CACHE_DURATION;
  });
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const [launchesData, rocketsData, starlinkData, upcomingData] = await Promise.all([
        getLaunches(),
        getRockets(),
        getStarlink(),
        getUpcomingLaunches(),
