...
function draw() {
  ...
  const parseYear = (d: Launch) =>
    d.date_utc ? new Date(d.date_utc).getFullYear() : null;
  const statusKey = (d: Launch) =>
    d.success ? "success" : !d.success ? "failure" : "unknown";

  const byYear = d3
    .rollups(
      hoverState.data.filter((d) => parseYear(d) !== null),
      (v) => {
        ...
        return {
          success: countsMap.get("success") ?? 0,
          failure: countsMap.get("failure") ?? 0,
          unknown: countsMap.get("unknown") ?? 0,
        };
      },
      parseYear as any
    )
  ...
  barGroups
    .selectAll("rect")
    .data((d) => d.map((p: any, i: number) => ({ key: (d as any).key, i, p })))
    .join("rect")
    .attr("x", (d) => axisX(String(byYear[d.i].year))!)
    .attr("y", (d) => axisY(d.p[1]))
    .attr("height", (d) => Math.max(0, axisY(d.p[0]) - axisY(d.p[1])))
    .attr("width", axisX.bandwidth())
    .on("mousemove", (event, d) => {
      const year = byYear[d.i].year;
      const total =
        byYear[d.i].success + byYear[d.i].failure + byYear[d.i].unknown;
      const count = byYear[d.i][
        d.key as keyof (typeof byYear)[number]
      ] as number;
...
function positionToolTip(event: MouseEvent, host: HTMLDivElement) {
  ...
  const tip = host.parentElement!.querySelector(
