...
const processedRockets = computed(() => {
  if (!spacexStore.rockets.length) return [ rows: [], max: {leo: 1, cost: 1, mass: 1}];
...
watch(processedRockets, (newData) => {
...
function updateChart(data: { rows: any[]; max: { leo: number; cost: number; mass: number } }) {
  option.value = {
    tooltip: {},
