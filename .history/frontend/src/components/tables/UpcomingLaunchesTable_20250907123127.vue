    ...
    <table v-else class="upcoming-table">
      ...
      <tbody>
        <tr v-for="launch in launches" :key="launch.id">
          ...
          <td>
            <span class="status-badge" :class="getStatusClass(launch.status?.name)">
...
watch(() => spacexStore.upcomingLaunches, (newLaunches) => {
...
function getStatusClass(status: string): string {
  ...
  if (statusLower.includes('tbd') || statusLower.includes('tentative')) return 'status-tbd';
